...
public class MaskPatterns {
    ...
    /**
     * Метод маскирует любое текстовое поле (сначала убирает пробелы из начала и конца):
     *  1) если 1 символ - заменит на "*****";
     *  2) если 2-4 символа - оставит первый, остальные заменит на "*****";
     *  3) если 5-9 символов - заменит на "*****" 60% символов, начиная со 2-го;
     *  4) если 10-15 символов - заменит на "*****" 60% символов, начиная с 3-го;
     *  5) если 16 символов - заменит на "*****" 60% символов, начиная с символа определяемого по формуле (length - charactersToMask)/2;
     *  6) вернет неизменной пустую строку.
     */
    ...
    /**
     * Метод маскирует PAN (сначала убирает пробелы из начала и конца):
     *  1) оставляет первые 6 и последние 4 символа, остальные заменятся на звездочки,
     *  2) вернет "*****" если меньше 11 символов,
     *  3) вернет неизменной пустую строку.
     */
    ...
    /**
     * Метод маскирует серию паспорта (сначала убирает пробелы из начала и конца):
     *  1) заменит последние 2 символа на звездочки,
     *  2) вернет "****" если меньше 3 символов,
     *  3) вернет неизменной пустую строку.
     */
    ...
    /**
     * Метод маскирует серию паспорта (сначала убирает пробелы из начала и конца):
     *  1) заменит первые 3 символа на звездочки,
     *  2) вернет "***" если меньше 4 символов,
     *  3) вернет неизменной пустую строку.
     */
    ...
    /**
     * Метод маскирует номер паспорта (сначала убирает пробелы из начала и конца):
     *  1) заменит все кроме 2 первых и 3 последних символов на звездочки,
     *  2) вернет "*****", если меньше 6 символов,
     *  3) вернет неизменной пустую строку.
     */
    ...
    /**
     * Метод заменит BigDecimal, Long, Double, Integer на 1111111111. Для других типов вернет исходное значение.
     */
    ...
    /**
     * Метод маскирует СНИЛС - уникальный номер из 11 чисел.
     * Перед началом маскировки метод обрабатывает строку, убирая пробелы из начала и конца строки. Если на входе пустая строка, или строка из пробелов, то она вернется в неизменном виде.
